...
mw.UploadWizardDetails = function( upload, api, containerDiv ) {
	...
	// MAP MODIFICATION
	_this.$locationMapContainer = $('<div id="map_'+_this.titleId+'" style="display:none;"></div>').addClass('mwe-map-container');
	_this.$showMapLink = $('<a href="#">'+mw.message( 'mwe-upwiz-location-showmap' ).text()+'</a>');
	...
	var locationDiv = $( '<div class="mwe-location mwe-upwiz-details-fieldname-input ui-helper-clearfix"></div>' )
		...
		).append(_this.$showMapLink).append(_this.$locationMapContainer); // MAP MODIFICATION
...
mw.UploadWizardDetails.prototype = {
	...
	attach: function() {
		if ( !this.isAttached ) {
			...
			this.updateCopyMsgs();
			this.updateMap(); // MAP MODIFICATION
		}
	},
	...
	updateMap: function() {
		...
		function onMapClick(e) {
			...
			_this.$lonInput.val(e.latlng.lng);
			map.addLayer(marker);
		};
		function onMarkerDragEnd(e) {
			_this.$latInput.val(e.target._latlng.lat);
			_this.$lonInput.val(e.target._latlng.lng);
		};
		var showMapClick = function(e) {
			...
			if(!map_initialized) {				
				...
				if (_this.$locationMapContainer.hasClass("map-opened")) {
					...
					_this.$locationMapContainer.removeClass("map-opened");
					_this.$showMapLink.text(mw.message( 'mwe-upwiz-location-showmap' ).text());
				}else {
					_this.$locationMapContainer.show();
					_this.$locationMapContainer.addClass("map-opened");
					_this.$showMapLink.text(mw.message( 'mwe-upwiz-location-hidemap' ).text());
		...
		_this.$showMapLink.click(showMapClick);
	},
